...
export class TeamCoordinator {
     ...
     * 并行调用多个专家
     */
    async callSpecialists(task, context, specialists) {
    ...
    async callSingleSpecialist(call) {
        const timeout = 60 * 1000; // 60 秒超时
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`[${call.name}] 调用超时（${timeout / 1000}s）`)), timeout));
        const work = async () => {
            // 创建专家 session
     ...
     * 根据专家结果生成整合报告
     */
    formatCoordinationReport(task, results, specialists) {
        const lines = [];
        lines.push(`## 📋 任务：${task}\n`);
