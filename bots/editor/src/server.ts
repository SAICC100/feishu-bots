...
import { getTenantAccessToken, replyMessage, handleChallenge } from "../../../shared/src/index.js";
...
async function main() {
  ...
  try {
    accessToken = await getTenantAccessToken(config.appId, config.appSecret);
    console.log("✅ 已获取 access token");
  } catch (e) {
    console.log("⚠️ 获取 token 失败，将在实际使用时重试");
  }
  ...
  app.get("/webhook", (req, res) => {
    const challengeResponse = handleChallenge(req.query as { challenge?: string });
    if (challengeResponse) {
      console.log("✅ 飞书 URL 验证通过");
